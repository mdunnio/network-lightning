...
module Network.Lightning.JsonRpc
    ( -- * Types
      JsonRpcT (..)
    , Method (..)
    , Request (..)
...
deriveJSON defaultOptions { omitNothingFields = True, fieldLabelModifier = snakeCase . drop 3 } ''Request
...
newtype JsonRpcT m a = JsonRpcT { unJsonRpcT :: StateT Int (ReaderT Socket m) a }
